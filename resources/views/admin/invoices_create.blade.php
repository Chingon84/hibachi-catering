...
@section('title', $mode === 'edit' ? 'Edit invoice' : 'Create invoice')

@push('styles')
<style>
  /* Page-specific full-screen invoice editor. Core chrome from app.css. */
  .create-shell{min-height:100vh;display:grid;grid-template-rows:auto 1fr;background:var(--card)}
  ...
  .btn-purple{display:inline-flex;align-items:center;justify-content:center;border:0;border-radius:8px;background:var(--brand);color:#fff;font-weight:850;padding:9px 13px;text-decoration:none;cursor:pointer}
  .btn-purple:hover{background:var(--brand-hover)}
  ...
  .memo-text{color:var(--brand);font-weight:850}
  ...
  .preview-tabs .active{color:var(--brand);border-bottom:2px solid var(--brand)}
  .paper{background:#fff;border-top:3px solid var(--brand);width:min(570px,100%);min-height:720px;margin:0 auto;box-shadow:0 18px 46px rgba(15,23,42,.16);padding:32px}
